...
def return_x_emulator (profile_type='xsb', feedback_type='ASN1', home='./X_emulator_profiles/', suite='IllustrisTNG', interpolation_type='linear'):
    '''
    Inputs:
    ...
    '''
    import x_helper_functions_1P as fs
...
feedback_type_list = ['OmegaM', 'sigma8', 'ASN1', 'ASN2', 'AAGN1', 'AAGN2'] 

for suite in suite_list :

    for f in feedback_type_list :
        for p in profiles :

            emulator_file =suite+'_1P_'+f+'_'+p+'_emulator.pkl' 
            if (not os.path.exists(emulator_file)) or rerun_building_emulator :

                radius_Mpc, x_emulator = return_x_emulator(profile_type=p, feedback_type=f, home=home, suite=suite)
