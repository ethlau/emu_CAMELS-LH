...
AAGN1=ASN1
AAGN2=ASN2
OmegaM=np.array([0.10, 0.14, 0.18, 0.22, 0.26, 0.30, 0.34, 0.38, 0.42, 0.46, 0.50])
Sigma8=np.array([0.60, 0.64, 0.68, 0.72, 0.76, 0.80, 0.84, 0.88, 0.92, 0.96, 1.00])
...
def choose_vary(vary_str):
    if vary_str=='OmegaM':
        vary = OmegaM
        nums=np.linspace(0,10,11,dtype='int')
    elif vary_str=='sigma8':
        vary = Sigma8
        nums=np.linspace(11,21,11,dtype='int')
    elif vary_str=='ASN1':
        vary=ASN1
        ...
        print('wrong parameter!', vary_str)
        exit(1)
    ...
    for s in np.arange(len(sims)):
        for n in np.arange(len(snap)):
            for m in np.arange(len(mass)):
                ...
                f=home+suite+'/'+suite+'_'+sims[s]+'_'+snap[n]+'_uw_'+mass_str[m]+'.txt'
                (x,  
                 mean_profiles_i['dens'], up_profiles_i['dens'], dn_profiles_i['dens'], std_profiles_i['dens'], median_profiles_i['dens'], 
                 mean_profiles_i['temp'], up_profiles_i['temp'], dn_profiles_i['temp'], std_profiles_i['temp'], median_profiles_i['temp'], 
                 mean_profiles_i['metal'], up_profiles_i['metal'], dn_profiles_i['metal'], std_profiles_i['metal'], median_profiles_i['metal'], 
...
def build_xsb_emulator(home,suite,vary_str,prof,func_str):
...
def build_emulator_3D(home,suite,vary_str,prof,func_str):
    ...
    print(x.shape, y.shape, samples.shape)
    emulator=ostrich.emulate.PcaEmulator.create_from_data(
        samples,
        y,
        ostrich.interpolate.RbfInterpolator,
